        ...
        <nav className="flex-1 px-3 py-6 space-y-7 overflow-y-auto custom-scrollbar">
          {menuItems.map((section, idx) => (
            <div key={idx} className="space-y-1.5">
              <h3 className="px-4 py-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-[0.15em] opacity-80">
                {section.section}
              </h3>
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Icon className={`w-4.5 h-4.5 transition-all duration-300 ${isActive ? 'text-cyan-400 scale-110' : 'text-slate-500 group-hover:text-slate-300'}`} />
